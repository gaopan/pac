<template>
  <div class="cust-dashboard-overview">
    <div class="container header-wrapper">
      <div class="header">
        ...
        <div class="header-actions" v-show="!moduleCreating">
          ...
          <button type="button" class="btn btn-primary" @click="toEditRemind" v-if="user.isAA || user.isAdmin" v-show="!isEditRemind&&!isEditSupport&&!isViewPastRS">编辑提醒</button>
          <button type="button" class="btn btn-primary" @click="toEditSupport" v-if="user.isBoss || user.isAdmin" v-show="!isEditRemind&&!isEditSupport&&!isViewPastRS">领导关注、支持事项</button>
        </div>
        ...
        <div class="col-xs-12 col-md-4 col-lg-3" v-for="m in modules" v-if="m.curMonthData && m.curMonthData.month">
          <div class="module" :class="{red: m.curMonthData.status=='r',yellow:m.curMonthData.status=='y',green:m.curMonthData.status=='g'}">
            <div class="head" @click="selectModule(m)">
              ...
              <div class="actions">
                <span @click="toEditData(m)"><i class="icon-edit-2"></i></span>
              </div>
            </div>
          </div>
        </div>
        <div class="col-xs-12 col-md-4 col-lg-3" v-if="(user.isAA ||user.isAdmin) && remoteModules.length < configuredModules.length">
    ...
    <div class="container" v-if="isEditRemind">
      <form>
        <div>
          <textarea class="form-control" v-model="rs.curMonthData.remind" :placeholder="'领导关注、支持事项'"></textarea>
        </div>
    ...
    <div class="container" v-if="isEditSupport">
      <form>
        ...
        <div>
          <textarea class="form-control" v-model="rs.curMonthData.support" :placeholder="'领导关注、支持'"></textarea>
        </div>
        <div class="form-actions">
